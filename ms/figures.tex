...
The stable rate was found by finding the root of $\frac{\partial \lambda_1}{\partial P}$, the derivative of the leading eigenvalue of the external stability matrix w.r.t to the rate of the invading modifier (see \emph{Supplemental Material SP6} for details). The dashed line shows $1-\frac{1}{k-1}$, which fit the values for $w=0.5$ (Carja et al. 2011). The values for $w=0.1$ (blue) could not be calculated for $k > 19$ due to numerical instability when selection is strong and the duration between selection fluctuations is long.
In all cases, $W=1$.}\label{fig:AkBk_stable_optimal_rate}
\end{figure}
